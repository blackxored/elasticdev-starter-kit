/* eslint-disable */
/*
 * This is pretty much vendor code so we can do fancy animations
 * in the future.
 */
...
class MaterialSharedElementTransitioner extends Component {
  ...
  getChildContext() {
    ...
    return {
      registerSharedView(sharedItem: SharedItem) {
        ...
        const matchingItem = self.state.sharedItems.findMatchByName(
          name,
          containerRouteName,
        );
  ...
  _getSharedElementStyle(props, prevProps, itemFrom, itemTo) {
    ...
    const animateFontSize = (itemFrom, itemTo) => {
      ...
      const getFontSize = element =>
        (element.props && element.props.fontSize) || 12;
      return {
        fontSize: progress.interpolate({
          inputRange: [0, 1],
          outputRange: [
            getFontSize(itemFrom.reactElement),
            getFontSize(itemTo.reactElement),
          ],
        }),
      };
    };
  ...
  _renderFakedSEContainer(pairs, props, prevProps) {
    ...
    const width = position.interpolate({
      inputRange,
      outputRange: [
        index > prevIndex ? fromItemBBox.width : toItemBBox.width,
        windowWidth,
      ],
    });
    const height = position.interpolate({
      inputRange,
      outputRange: [
        index > prevIndex ? fromItemBBox.height : toItemBBox.height,
        windowHeight,
      ],
    });
  ...
  _renderOverlay(props: any, prevProps: any) {
    const fromRoute = prevProps
      ? prevProps.scene.route.routeName
      : 'unknownRoute';
    const toRoute = props.scene.route.routeName;
    const pairs = this.state.sharedItems.getMeasuredItemPairs(
      fromRoute,
      toRoute,
    );
    const sharedElements = pairs.map((pair, idx) => {
      const { fromItem, toItem } = pair;
      const animatedStyle = this._getSharedElementStyle(
        props,
        prevProps,
        fromItem,
        toItem,
      ...
      return React.createElement(
        ...
        {
          ...element.props,
          style: [element.props.style, animatedStyle],
          key: idx,
        },
    ...
    console.log(
      ...
      pairsStr,
      'items:',
      this.state.sharedItems._items
        .filter(i => i.metrics)
        .map(
          i => `${i.name} ${i.containerRouteName} ${JSON.stringify(i.metrics)}`,
        ),
    );
    console.log(
      this.state.sharedItems._items.map(
        i => `${i.name} ${i.containerRouteName} ${JSON.stringify(i.metrics)}`,
      ),
    );
    // if (pairs.length > 0) {
